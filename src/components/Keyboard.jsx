...
class KeyB extends React.Component {
	...
	constructor(props) {
		...
		this.persistKataResults = this.persistKataResults.bind(this);
		this.saveKataResults = this.saveKataResults.bind(this);
		this.cambiaFiltro = this.cambiaFiltro.bind(this);
		this.resetKata = this.resetKata.bind(this);
	}
	...
	async saveKataResults() {
		...
		const body = JSON.stringify({
			...
			seconds: segundos,
			velocity: vel,
			text: this.state.textoJuego,
		});
		if (this.state.currentUser) {
			const header = new Headers(authHeader());
			header.set('Accept', 'application/json');
			header.set('Content-Type', 'application/json');
			await fetch("http://localhost:4500/api/v1/assessment", {
				method: "POST",
				headers: header,
				body: body,
			});
	...
	async loadRandomText(reinicia) {
		...
		const res = await fetch(`http://localhost:4500/api/v1/kata/random/${jType}`, {
			method: "GET",
			headers: {
	...
	avanza(lugares, rojo) {
		...
		for (i = 0; i < lugares; i++) {
			if (i + 1 === lugares && rojo === 1) {
				...
				if (temp[i] === " ") {
					...
					textoForm += "<span class='malReturn'>[Enter]</span><br>";
					break;
				} else if (temp[i] === "<") {
					textoForm += "&lt";
				} else if (temp[i] === ">") {
			...
			switch (temp[i]) {
				case "\t":
					textoForm += "<span class='" + malTab + "'>[TAB]</span>";
					break;
				case "\n":
					textoForm += "<br>";
					break;
		...
		for (i = lugares; i < temp.length; i++) {
			...
			switch (temp[i]) {
				case "\t":
					textoForm += "<span class='iTab'>[TAB]</span>";
					break;
				/*case '\n':
	...
	async cambiaText() {
		...
		if (texto != textoViejo) {
			...
			var it = 0;
			if (ite >= ndown - 1) {
				for (var i = texto.length - 1; i >= 0 && j < ndown && ndown >= 2; i--) {
					it = i;
					switch (ndown) {
						case 2:
							it = i;
	...
	animaKeyboard(tecla, color) {
		$(tecla).animate({ backgroundColor: color }, 0, "swing", function () {
	...
	animaKey(keycode, color) {
		switch (keycode) {
			...
			case "ª":
			case "\\":
				this.animaKeyboard(".grad", color);
				break;
			case "0":
			case "=":
			case "≠":
				this.animaKeyboard(".n0", color);
				break;
			case "1":
			case "!":
			case "|":
				this.animaKeyboard(".n1", color);
				break;
			case "2":
			case '"':
			case "@":
				this.animaKeyboard(".n2", color);
				break;
			case "3":
			case "·":
			case "#":
				this.animaKeyboard(".n3", color);
				break;
			case "4":
			case "$":
			case "¢":
				this.animaKeyboard(".n4", color);
				break;
			case "5":
			case "%":
			case "∞":
				this.animaKeyboard(".n5", color);
				break;
			case "6":
			case "&":
			case "¬":
				this.animaKeyboard(".n6", color);
				break;
			case "7":
			case "/":
			case "÷":
				this.animaKeyboard(".n7", color);
				break;
			case "8":
			case "(":
				this.animaKeyboard(".n8", color);
				break;
			case "9":
			case ")":
				this.animaKeyboard(".n9", color);
				break;
			case "?":
			case "'":
				this.animaKeyboard(".npDown", color);
				break;
			case "¿":
			case "¡":
				this.animaKeyboard(".npUp", color);
				break;

			case "q":
			case "Q":
				this.animaKeyboard(".lq", color);
				break;
			case "w":
			case "W":
				this.animaKeyboard(".lw", color);
				break;
			case "e":
			case "é":
			case "E":
			case "É":
				this.animaKeyboard(".le", color);
				break;
			case "r":
			case "R":
				this.animaKeyboard(".lr", color);
				break;
			case "t":
			case "T":
				this.animaKeyboard(".lt", color);
				break;
			case "y":
			case "Y":
				this.animaKeyboard(".ly", color);
				break;
			case "u":
			case "ú":
			case "U":
			case "Ú":
				this.animaKeyboard(".lu", color);
				break;
			case "i":
			case "í":
			case "I":
			case "Í":
				this.animaKeyboard(".li", color);
				break;
			case "o":
			case "ó":
			case "O":
			case "Ó":
				this.animaKeyboard(".lo", color);
				break;
			case "p":
			case "P":
				this.animaKeyboard(".lp", color);
				break;
			case "`":
			case "^":
			case "[":
				this.animaKeyboard(".lainv", color);
				break;
			case "+":
			case "*":
			case "]":
				this.animaKeyboard(".last", color);
				break;
			...
			case "A":
			case "Á":
				this.animaKeyboard(".la", color);
				break;
			case "s":
			case "S":
				this.animaKeyboard(".ls", color);
				break;
			case "d":
			case "D":
				this.animaKeyboard(".ld", color);
				break;
			case "f":
			case "F":
				this.animaKeyboard(".lf", color);
				break;
			case "g":
			case "G":
				this.animaKeyboard(".lg", color);
				break;
			case "h":
			case "H":
				this.animaKeyboard(".lh", color);
				break;
			case "j":
			case "J":
				this.animaKeyboard(".lj", color);
				break;
			case "k":
			case "K":
				this.animaKeyboard(".lk", color);
				break;
			case "l":
			case "L":
				this.animaKeyboard(".ll", color);
				break;
			case "ñ":
			case "Ñ":
				this.animaKeyboard(".lñ", color);
				break;
			case "´":
			case "¨":
			case "{":
				this.animaKeyboard(".lllaa", color);
				break;
			case "ç":
			case "Ç":
			case "}":
				this.animaKeyboard(".lllac", color);
				break;

			case "<":
			case ">":
			case "≤":
				this.animaKeyboard(".lmm", color);
				break;
			case "z":
			case "Z":
				this.animaKeyboard(".lz", color);
				break;
			case "x":
			case "X":
				this.animaKeyboard(".lx", color);
				break;
			case "c":
			case "C":
				this.animaKeyboard(".lc", color);
				break;
			case "v":
			case "V":
				this.animaKeyboard(".lv", color);
				break;
			case "b":
			case "B":
				this.animaKeyboard(".lb", color);
				break;
			case "n":
			case "N":
				this.animaKeyboard(".ln", color);
				break;
			case "m":
			case "M":
				this.animaKeyboard(".lm", color);
				break;
			case ",":
			case ";":
				this.animaKeyboard(".lcoma", color);
				break;
			case ".":
			case ":":
				this.animaKeyboard(".lpunto", color);
				break;
			case "-":
			case "_":
				this.animaKeyboard(".lmenos", color);
				break;

			case " ":
				this.animaKeyboard(".esp", color);
				break;
			default:
				console.log(keycode);
		}
				...
				<div className="container-fluid">
					<div className="row">
						...
						<div className="col-lg-6 col-md-6 col-sm-6 col-xs-6">
							<button
								className="btn btn-success btnMTC btnMTChide"
								onClick={this.startWatch}
							>
								Comenzar
							</button>
							<button
								className="btn btn-primary btnMTC"
								onClick={this.pauseWatch}
							>
								Pausa
							</button>
							{currentUser &&
							<button
								className="btn btn-primary btnMTC"
								onClick={this.persistKataResults}
							>
								Guardar Sesión
							</button>
							}
							<button
								className="btn btn-warning btnMTC"
								onClick={this.resetKata}
							>
								Reiniciar
							</button>
							...
							<div className="container-fluid mb-0">
								<div className="row mt-0 mb-0">
									<div className="col-6 colCorrecto">
										...
										<p id="pCorrectos" className="pC mb-0">
											0
										</p>
									</div>
									<div className="col-6 colIncorrecto">
										<p className="mb-0">Incorrectos</p>
										<p id="pIncorrectos" className="pC mb-0">
											0
										</p>
									</div>
								</div>
